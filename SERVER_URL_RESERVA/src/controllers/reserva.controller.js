...
const verificarHoraFinDentroRango = (
  horaInicio,
  duracion,
...
const enviarMensajeWhatsApp = async (telefono, mensaje) => {
  try {
    ...
    if (response.status === 200) {
      ...
      console.error(
        `Error al enviar el mensaje a ${telefono}: ${response.statusText}`
      );
    }
  } catch (error) {
...
export const registrarReserva = async (req, res) => {
  const { pacienteId, medicoId, especialidadId, fechaReserva, horaInicio } =
    req.body;
  ...
  if (
    ...
    !mongoose.Types.ObjectId.isValid(especialidadId)
  ) {
    return res.status(400).json({
      response: "error",
      message: "ID de paciente, médico o especialidad inválido."
    });
  }
  ...
  try {
    ...
    if (!paciente) {
      return res
        .status(404)
        .json({ response: "error", message: "Paciente no encontrado." });
    }
    ...
    const tieneEspecialidad = medico.especialidades.some((especialidad) =>
      especialidad.equals(especialidadId)
    );
    if (!tieneEspecialidad) {
      return res.status(400).json({
        response: "error",
        message: "El médico no tiene la especialidad solicitada."
      });
    }
    ...
    if (!disponibilidad) {
      return res.status(400).json({
        response: "error",
        message: "El médico no tiene disponibilidad para esta especialidad."
      });
    }
    ...
    const duracion = especialidad.name
      .toLowerCase()
      .includes("medicina general")
      ? 20
      : 30;
    ...
    if (
      !verificarHoraFinDentroRango(horaInicio, duracion, disponibilidad.fin)
    ) {
      return res.status(400).json({
        response: "error",
    ...
    // Si la especialidad es "MEDICINA GENERAL", confirmar automáticamente
    if (especialidad.name.toLowerCase() === "medicina general") {
      nuevaReserva.estadoConfirmacionMedico = "confirmado";
      nuevaReserva.estadoConfirmacionPaciente = "confirmado";

      // Enviar mensaje solo al paciente
      const mensajePaciente =
        `👋👨‍⚕️ *Hola ${paciente.name} ${paciente.lastname}*,\n\n` +
        `📅 Tu reserva con el médico *${medico.name} ${medico.lastname}* en la especialidad de *${especialidad.name}* ha sido registrada exitosamente.\n\n` +
        `⏰ Fecha de la cita: *${format(
      ...
      // Mensajes para paciente y médico en otras especialidades
      const mensajePacientePendiente =
        `👋👨‍⚕️ *Hola ${paciente.name} ${paciente.lastname}*,\n\n` +
      ...
      await enviarMensajeWhatsApp(paciente.telefono, mensajePacientePendiente);
      await enviarMensajeWhatsApp(medico.telefono, mensajeMedicoPendiente);
    }
    ...
    return res.status(500).json({
      response: "error",
      message: "Error del servidor al registrar la reserva."
    });
  }
};
...
export const getCitas = async (req, res) => {
  try {
    ...
    if (citas.length === 0) {
      return res
        .status(404)
        .json({ response: "error", message: "No se encontraron citas." });
    }
...
export const getCitaById = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(citaId)) {
    return res
      .status(400)
      .json({ response: "error", message: "ID de cita inválido." });
  }
  ...
  try {
    ...
    if (!cita) {
      return res
        .status(404)
        .json({ response: "error", message: "Cita no encontrada." });
    }
...
export const eliminarCita = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      response: "error",
      message: "Error del servidor al eliminar la cita."
    });
  }
};

// Controlador para que el médico confirme o cancele la reserva
export const confirmarOCancelarReserva = async (req, res) => {
  ...
  if (!["confirmado", "cancelado"].includes(estadoConfirmacionMedico)) {
    return res.status(400).json({
      response: "error",
      message: "El estado de confirmación debe ser 'confirmado' o 'cancelado'."
    });
  }
  ...
  try {
    ...
    if (!reserva) {
      return res
        .status(404)
        .json({ response: "error", message: "Reserva no encontrada." });
    }
    ...
    if (estadoConfirmacionMedico === "confirmado") {
      ...
      const mensajeMedico =
        `👋👨‍⚕️ *Gracias Dr. ${reserva.medico.name} ${reserva.medico.lastname}*,\n\n` +
        `✅ Por confirmar tu reserva con el paciente *${
          reserva.paciente.name
        } ${reserva.paciente.lastname}* para la fecha *${format(
      ...
      const mensajePaciente =
        `👋👨‍⚕️ *Hola ${reserva.paciente.name} ${reserva.paciente.lastname}*,\n\n` +
      ...
      // Mensaje para el médico
      const mensajeMedico =
        `👋👨‍⚕️ *Hola Dr. ${reserva.medico.name} ${reserva.medico.lastname}*,\n\n` +
        `❌ Has cancelado la reserva con el paciente *${
          reserva.paciente.name
        } ${reserva.paciente.lastname}* programada para la fecha *${format(
          new Date(reserva.fechaReserva),
          "EEEE d 'de' MMMM",
          { locale: es }
        )}*.\n`;
    ...
    return res.status(500).json({
      response: "error",
      message: "Error del servidor al confirmar o cancelar la reserva."
    });
  }
};
